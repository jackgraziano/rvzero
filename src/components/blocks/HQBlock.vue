<template>
  <div class="hq-block">
    <ComparisonBlockHeader
      ...
      title="BLOCO HQ — RESTRIÇÕES DE VAZÃO"
                  ...
                  >
                    <div v-if="row.valores[col.key]?.valor1" class="restricao-details">
                      <div class="limites-section" v-if="row.valores[col.key].valor1.limites">
                        ...
                        <div class="limite-row">P: {{ formatRange(row.valores[col.key].valor1.limites.pesado_min, row.valores[col.key].valor1.limites.pesado_max) }}</div>
                        <div class="limite-row">M: {{ formatRange(row.valores[col.key].valor1.limites.medio_min, row.valores[col.key].valor1.limites.medio_max) }}</div>
                        <div class="limite-row">L: {{ formatRange(row.valores[col.key].valor1.limites.leve_min, row.valores[col.key].valor1.limites.leve_max) }}</div>
                      </div>
                  ...
                  >
                    <div v-if="row.valores[col.key]?.valor2" class="restricao-details">
                      <div class="limites-section" v-if="row.valores[col.key].valor2.limites">
                        <strong>Limites:</strong>
                        <div class="limite-row">P: {{ formatRange(row.valores[col.key].valor2.limites.pesado_min, row.valores[col.key].valor2.limites.pesado_max) }}</div>
                        <div class="limite-row">M: {{ formatRange(row.valores[col.key].valor2.limites.medio_min, row.valores[col.key].valor2.limites.medio_max) }}</div>
                        <div class="limite-row">L: {{ formatRange(row.valores[col.key].valor2.limites.leve_min, row.valores[col.key].valor2.limites.leve_max) }}</div>
                      </div>
                      <div class="coeficientes-section">
...
<script>
import { formatRange, semanticEqual } from '../../utils/comparison.js'
